...
function init() {
    ...
    listar_ventas();//llama la lista de productos en ventana modal en ventas.php
...
function listar_ventas(){
    tabla_compras=$('#lista_productos_ventas_data').dataTable({
        "aProcessing":true,//Activamos el procesamiento del datatables
        "aServerSide":true,//Paginacion y filtrado realizados por el servidor
        dom:'Bfrtip',//Definimos los elementos del control de tabla
        buttons: [
        ...
        "ajax":{
            url:'../ajax/producto.php?op=listar_ventas',
            type:"get",
            dataType:"json",
            error:function(e){
                console.log(e.responseText);
...
/*CARGAR PRODUCTO, PRECIO, CANTIDAD, IGV, IMPORTE EN VENTAS
	...
	*Asi que detalles pertenece al arreglo detalles[]
	*Para agregar elementos a un arreglo en javascript, se utiliza el metodo push()
    *Puedes agregar variables u objetos, lo que yo he hechos es agregarle un 
    objeto y ese objeto que contiene mucha informacion, que sería como una fila con muchas columnas
	*Para crear un objeto de ese tipo (con columnas), se utliliza esto :
	var obj = { }
	*Dentro de las llaves definas columna y valor (Todo esto para una fila)
	*Lo defines asi:
	nombre_columna : valor
	*El lenght sirve para calcular la longitud del arreglo o el 
	numero de objetos que tiene el arreglo, que es lo mismo Y es por eso que 
	lo necesito en el for. Claro que puedes agregarle un id y name al td*/
...
function agregarDetalleVenta(id_producto,producto,estado){
	        $.ajax({
		            url:"../ajax/producto.php?op=buscar_producto_venta",
					 method:"POST",
					//data:dataString,
					//toma el valor del id y del estado
					data:{id_producto:id_producto, producto:producto,estado:estado},
                ...
                /*IMPORTANTE: var obj: es un objeto con mucha informacion que 
                contiene una fila con muchas columnas
				Para crear un objeto de ese tipo (con columnas), se utliliza esto :
		        var obj = { }, Dentro de las llaves definas columna y valor (Todo esto para una fila)
				Lo defines asi:
				nombre_columna : valor 
         ...
         /*IMPORTANTE: detalles.push(obj);: Para agregar elementos a 
            un arreglo en javascript, se utiliza el metodo push()
            *Puedes agregar variables u objetos, lo que yo he hechos 
            es agregarle un objeto y ese objeto que contiene mucha informacion,
            *el detalles de detalles.push(obj); viene de detalles = [],
             una vez se agrega el objeto al arreglo entonces se llama 
             a la function listarDetalles(); */
						detalles.push(obj);
  	...
  	for(var i=0; i<detalles.length; i++){
		if(detalles[i].estado == 1){ 
            ...
            var filas = filas + "<tr><td>"+(i+1)+"</td> <td name='producto[]'>"+detalles[i].producto+
                    ...
                    "</td> <td>"+detalles[i].stock+"</td> <td> <input type='text' class='cantidad' name='cantidad[]' id=cantidad_"+
                    i+" onClick='setCantidad(event, this, "+(i)+");' onKeyUp='setCantidadAjax(event, this, "+
                    (i)+");' value='"+detalles[i].cantidad+"'> </td>  <td><input type='text' name='descuento[]'"+
                    " id='descuento[]' onClick='setDescuento(event, this, "+(i)+");' onKeyUp='setDescuento(event, this, "+(i)+
                    ");' value='"+detalles[i].dscto+"'></td> <td> <span name='importe[]' id=importe"+i+">"+detalles[i].moneda+" "+
                    detalles[i].importe+"</span> </td> <td>  <button href='#' class='btn btn-danger btn-lg' role='button'"+
                    " onClick='eliminarProd(event, "+(i)+");' aria-pressed='true'><span class='glyphicon glyphicon-trash'></span> </button></td>   </tr>";
         ...
         success:function(data){
             ...
             //console.log(data);
              ...
              $("#resultados_ventas_ajax").html(data);
                ...
                /*se pone isNaN porque al ser vacio indica que no es un numero, entonces si
                 valida que es cierto entonces se desabilita el boton del envio del formulario
                  y de agregar productos
                si la cantidad a vender es igual a cero o a vacio o si es mayor al stock 
                entonces se desabilita el boton de enviar formulario y de agregar productos*/
                ...
                // $("div[id=resultados_ajax]").remove();
                     ...
                     /*despues de eliminar agrega el id del mensaje de ajax,
                      ya que se habia removido el mensaje "campo vacio" en la funcion eliminar*/
...
//Funcion que asigna el descuento en la venta
function setDescuento(event, obj, idx){
  	event.preventDefault();
  	detalles[idx].dscto = parseFloat(obj.value);
  	recalcular(idx);
...
//Funcion que actualiza los calculos al agregar un producto
function recalcular(idx){
  	//alert('holaaa:::' + obj.value);
  	//var asd = document.getElementById('cantidad');
  	//console.log(detalles[idx].cantidad);
    ...
    /*IMPORTANTE:porque cuando agregaba una segunda fila el importe
    se alteraba? El importe se modificaba por que olvidé restarle
    el descuento Así que solo agregué esa resta a la operación*/
...
//Funcion que calcula los valores totales de la venta
function calcularTotales(){	
  	var subtotal = 0;
  	var total = 0;
    var subtotalFinal = 0;
...
/* {'arrayCompra':JSON.stringify(detalles)}:Esa parte encapsula el 
arreglo detalles y lo envía como un solo parametro
*/
...
function registrarVenta(){
    ...
    $.ajax({
		...
		method:"POST",
        data:{'arrayVenta':JSON.stringify(detalles), 'numero_venta':numero_venta,
            'cedula':cedula,'nombre':nombre, 'apellido':apellido,'direccion':direccion,
            'total':total,'vendedor':vendedor,'tipo_pago':tipo_pago,'id_usuario':id_usuario,
