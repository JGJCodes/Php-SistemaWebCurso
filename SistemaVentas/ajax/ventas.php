 ...
 * de la informacion tabla Ventas por medio de ajax
 */

  require_once("../config/conexion.php");//llamo a la conexion de la base de datos 
  require_once("../models/venta.php"); //llamo al modelo Venta
  ...
  switch($_GET["op"]){
    ...
    case "detalle_cliente":
                ...
                // si existe el cliente entonces recorre el array
    ...
    case "buscar_ventas":
            ...
            foreach($datos as $row){
                ...
                $sub_array[] = '<button class="btn btn-warning detalle"  id="'.
                                $row["numero_venta"].'"  data-toggle="modal" '.
                               ' data-target="#detalle_venta"><i class="fa fa-eye"></i></button>';
            ...
            //IMPORTANTE: poner \' cuando no sea numero, sino no imprime*/
                $sub_array[] = '<button type="button" onClick="cambiarEstado('.
                                $row["id_ventas"].',\''.$row["numero_venta"].
                                '\','.$row["estado"].');" name="estado" id="'.
                                $row["id_ventas"].'" class="'.$atrib.'">'.
                                $est.'</button>';
                $data[] = $sub_array;
    ...
    case "buscar_ventas_fecha":
            ...
            foreach($datos as $row) {
                ...
                //IMPORTANTE: poner \' cuando no sea numero, sino no imprime*/
                $sub_array[] = '<button type="button" onClick="cambiarEstado('.$row["id_ventas"].
                                ',\''.$row["numero_venta"].'\','.$row["estado"].');" name="estado" id="'.
                                $row["id_ventas"].'" class="'.$atrib.'">'.$est.'</button>';        
                $data[] = $sub_array;
            }//Cierre ciclo for
    ...
    case "buscar_ventas_fecha_mes": 
            ...
            foreach($datos as $row) {
                  ...
                  //IMPORTANTE: poner \' cuando no sea numero, sino no imprime*/
                    $sub_array[] = '<button type="button" onClick="cambiarEstado('.
                                    $row["id_ventas"].',\''.$row["numero_venta"].'\','.
                                    $row["estado"].');" name="estado" id="'.$row["id_ventas"].
    ...
    case "consulta_cantidad_venta":
        ...
        if(is_array($datos)==true and count($datos)>0){
            ...
            foreach($datos as $row) {               
                ...
                /*consultamos si la cantidad que se va a querer 
                vender es mayor a la cantidad de stock entonces 
                que solo se refleje la cantidad maxima que se 
                encuentre en el stock y que me devuelva ese
                 valor en el campo*/
