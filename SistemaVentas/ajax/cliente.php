...
/*declaramos las variables de los valores que se envian por el 
    ...
    *el valor id_usuario y cedula_cliente se carga en el campo
     hidden cuando se edita un registro
               ...
               si ya existe un registro con el cliente entonces no se registra*/
                       if(is_array($datos)==true and count($datos)==0){
                            ...
                            $clientes->registrar_cliente($cedula,$nombre,$apellido,
                                        $telefono,$correo,$direccion,$estado,$id_usuario);
                ...
                $clientes->editar_cliente($cedula,$nombre,$apellido,$telefono,
                                        $correo,$direccion,$estado,$id_usuario);
            ...
            $atrib = "btn btn-success btn-md estado";
             ...
             if($row["estado"] == 0){
              ...
              $sub_array[] = '<button type="button" onClick="cambiarEstado('.
                            $row["id_cliente"].','.$row["estado"].');" name="estado" id="'.
                            $row["id_cliente"].'" class="'.$atrib.'">'.$est.'</button>';
              $sub_array[] = '<button type="button" onClick="mostrar('.$row["cedula_cliente"].
                            ');" id="'.$row["id_cliente"].'" class="btn btn-warning btn-md">'.
                            '<i class="glyphicon glyphicon-edit"></i> Editar</button>';
              $sub_array[] = '<button type="button" onClick="eliminar('.$row["id_cliente"].
                            ');" id="'.$row["id_cliente"].'" class="btn btn-danger btn-md">'.
        ...
        foreach($datos as $row){
             ...
             if($row["estado"] == 0){
              ...
              $sub_array[] = '<button type="button"  name="estado" id="'.
                        $row["id_cliente"].'" class="'.$atrib.'">'.$est.'</button>';
              $sub_array[] = '<button type="button" onClick="agregar_registro('.
                            $row["id_cliente"].','.$row["estado"].');" id="'.$row["id_cliente"].
                            '" class="btn btn-primary btn-md"><i class="fa fa-plus"'.
